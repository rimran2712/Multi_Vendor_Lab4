...
This program will tell us which intercaes of devices are connected and their IP addresses.
"""
""""
...
ipdb> nr.inventory.hosts['vIOS1']['interfaces_ip_info']
{'get_interfaces_ip': {'GigabitEthernet0/0': {'ipv4': {'172.16.100.2': {'prefix_length': 30}}}}}
...
"""
def get_interafces_info (task):
    # This function will get interfaces details and store into dictionay keys and embend into inventory
...
nr.run (task=get_interafces_info)

def connected_up_interafces (task):
    # This function will provide us interface connected and status is up
    interfaces = task.host['interfaces_info']['get_interfaces']
    for interface in interfaces.keys():
        if interfaces[interface]['is_up'] == True:
            print (f"{task.host}  interface *** {interface} *** is UP")

nr.run (task=connected_up_interafces)
